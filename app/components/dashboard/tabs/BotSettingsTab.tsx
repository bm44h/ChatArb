...
export default function BotSettingsTab({ projectId }: { projectId: string }) {
    ...
    const fetchSettings = async () => {
      ...
      try {
        ...
        const currentData = {
          ...
          welcomeMessage: data.settings?.welcomeMessage || 'أهلاً بك في متجرنا كيف يمكننا مساعدتك؟',
  ...
  useEffect(() => {
    const currentData = { storeName, botName, storeUrl, welcomeMessage, chatColor };
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(responseData.message || 'فشل في حفظ الإعدادات');
    ...
    <div className="p-6 bg-white rounded-lg border border-gray-200 space-y-8">
      ...
      <div>
        ...
        <div className="space-y-6">
          <div>
            <label className="block text-sm font-medium text-gray-800 mb-2">الرسالة الترحيبية</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-800 mb-4">لون نافذة الدردشة</label>
              ...
              {colorOptions.map(option => (
                <button
                  ...
                  className={`w-10 h-10 rounded-full transition-all duration-200 ${
                    chatColor === option.value ? 'ring-2 ring-offset-2 ring-blue-500' : ''
                  }`}
      ...
      <div>
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          >
            <RefreshCw className={`inline ml-2 h-4 w-4 ${isSyncing ? 'animate-spin' : ''}`} />
            {isSyncing ? 'جاري الزحف...' : 'زحف'}
          </button>
        </div>
        <div>
            <label className="block text-sm font-medium text-gray-700">رابط المتجر</label>
            <input
              type="url"
              value={storeUrl}
