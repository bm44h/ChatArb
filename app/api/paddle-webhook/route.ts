...
// [تم التعديل] استيراد createClient بدلاً من createServerActionClient
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (event.event_type === 'transaction.completed') {
      ...
      if (!userId || !projectName || !planId || !periodStartsAt || !periodEndsAt) {
        console.error('Webhook missing critical data:', { userId, projectName, planId, periodStartsAt, periodEndsAt });
        return new NextResponse('Webhook missing critical data', { status: 400 });
      }
      ...
      if (projectError) {
        ...
        return new NextResponse(`Error creating project: ${projectError.message}`, { status: 500 });
      }
      ...
      if (subscriptionError) {
        console.error('Error creating subscription from webhook:', subscriptionError);
        return new NextResponse(`Error creating subscription: ${subscriptionError.message}`, { status: 500 });
      }

      console.log(`Successfully created project ${newProject.id} with plan ${planId} for user ${userId}`);
    }
